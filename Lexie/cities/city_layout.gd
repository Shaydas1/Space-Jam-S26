...
func get_street(x : int, y : int, horizontal : bool) -> bool:
    var arr = horizontal_streets if horizontal else vertical_streets

    if (x < 0) or (x >= size.x + 1) or (y < 0) or (y >= size.y + 1):
        return false
    
    return arr[x + y * size.x]
